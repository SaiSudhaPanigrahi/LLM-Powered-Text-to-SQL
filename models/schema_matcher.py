...
#         # Precompute embeddings using formatted schema text
#         self.db_texts = []
#         self.db_ids = []
#         for schema in self.schemas:
#             db_id = schema["db_id"]
#             schema_text = self._format_schema(schema)  # minimal formatting for matching
#             self.db_texts.append(schema_text)
#             self.db_ids.append(db_id)
...
#     def _format_schema(self, schema_obj):
#         # Flatten table + column names minimally for embedding matching only
#         lines = []
#         for i, table in enumerate(schema_obj['table_names_original']):
#             columns = [col[1] for col in schema_obj['column_names_original'] if col[0] == i and col[1] != "*"]
#             if columns:
